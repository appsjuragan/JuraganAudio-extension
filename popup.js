...
function scope() {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        i.onclick = function () {
            if (p && userPresetSpan.children.length >= p + 1) {
                a("Saving more than 3 presets requires Ears Pro. Click the Pro tab above to start your free trial!");
                return;
            }
            var e = n.value.trim();
            if (e != "") {
        ...
        s.onclick = function () {
            chrome.runtime.sendMessage({ type: "exportPresets" });
        };
        ...
        l.onclick = function () {
            if (p) {
                a("The frequency spectrum visualizer requires Ears Pro. Click the Pro tab above to start your free trial!");
            }
            H();
            f();
        };
    ...
    var O = "eqSvg";
    var z = "eqTabButton";
    var A = "How was your day of beautiful audio? Let us know what you think at the link below! If you would like to continue using Ears, please support us by making a purchase. If you have already purchased Ears, please restart Chrome. Enjoy!";
    var D = "https://chrome.google.com/webstore/detail/ears-audio-toolkit/nfdfiepdkbnoanddpianalelglmfooik";
    var i = true;
    ...
    function $(e, t) {
        ...
        var r = I.rect(0, 0, T, B).attr({ stroke: m });
        re(I, e);
        ae(I);
        var a = x.line(g / 2, F(k), g / 2, F(M)).attr({ stroke: "wheat", opacity: 0.5 });
        var i = x
            .text(g / 2, F(M) - 10, "volume")
            .attr({ fill: "wheat", "text-anchor": "middle", "font-size": 8 });
        var o = x.line(g / 2 - 5, F(0), g / 2 + 5, F(0)).attr({ stroke: "wheat" });
        var s = x
            .line(0, t.y, g, t.y)
            .attr({ stroke: "wheat" })
            .addClass("gainLine");
    ...
    function d(e, t, n) {
        ...
        for (var g = 0; g < T; g += 2) {
            ...
            var M = Math.log(
                (Math.pow(u + l + f, 2) - 4 * (u * l + 4 * u * f + l * f) * y + 16 * u * f * y * y) /
                (Math.pow(1 + v + d, 2) - 4 * (v + 4 * d + v * d) * y + 16 * d * y * y)
            );
            M = (M * 10) / Math.LN10;
            M = F(M);
            if (M == -Infinity) {
        ...
        R[n] = e.polyline(m).attr({ stroke: k, "fill-opacity": "0", "pointer-events": "none" });
    }
    ...
    function ae(e) {
        for (var t = 5; t < c; t *= 2) {
            var n = P(t);
            e.line(n, B / 2 + 10, n, B / 2 - 10).attr({ stroke: "wheat", "stroke-opacity": 0.25 });
            e.line(n, B, n, B - 15).attr({ stroke: "wheat" });
            e
                .text(n, B - 18, "" + Math.round(l(n)))
    ...
    function se(e) {
        return function () {
            this.attr({ fill: q });
            chrome.runtime.sendMessage({ type: "gainUpdated", gain: e.gain });
            n();
        };
    }
    ...
    function ce(o, s, c) {
        return function (e, t, n, r, a) {
            if (a.shiftKey) {
                ...
                this.attr({
                    transform:
                        this.data("origTransform") + (this.data("origTransform") ? "T" : "t") + [e, t]
                });
            }
            d(c, o, s);
            ue(o, s);
    ...
    function ve(e) {
        chrome.runtime.sendMessage({ type: "modifyGain", gain: e.gain });
    }

    function de(e) {
        chrome.runtime.sendMessage({ type: "resetFilter", index: e });
    }
